...
class QHLine(QtWidgets.QFrame):
    def __init__(self, width=2, color="black"):
        super(QHLine, self).__init__()
        self.setFrameShape(QtWidgets.QFrame.HLine)
        self.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.setLineWidth(width)
        self.setStyleSheet("color: "+color+"; background-color: black;")
...
def loadStyleSheet(filedir):
    with open(filedir, 'r') as myfile:
        stylesheet = myfile.read().replace('\n', '')
    return stylesheet

def set_Icon(QElement, iconname, QType='QPushButton', filetype='png'):
    stylesheet = QType+" { image: url(icons/"+iconname+"."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":hover { image: url(icons/"+iconname+"_hovered."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":pressed { image: url(icons/"+iconname+"."+filetype + \
        "); background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
    QElement.setStyleSheet(stylesheet)


def set_IconChecked(QElement, iconname, QType='QPushButton', filetype='png'):
    stylesheet = QType+" { image: url(icons/"+iconname+"."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":hover { image: url(icons/"+iconname+"_hovered."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":checked:hover { image: url(icons/"+iconname+"."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":checked { image: url(icons/"+iconname+"_hovered."+filetype + \
        "); background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
    stylesheet = stylesheet+QType + \
        ":pressed { image: url(icons/"+iconname+"."+filetype + \
        ");  background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
...
def set_Icon2Checked(QElement, iconname, QType='QPushButton', filetype='png'):
    stylesheet = QType+" { image: url(icons/"+iconname+"_off."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":hover { image: url(icons/"+iconname+"_off_hovered."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":checked:hover { image: url(icons/"+iconname+"_on_hovered."+filetype+");}"
    stylesheet = stylesheet+QType + \
        ":checked { image: url(icons/"+iconname+"_on."+filetype + \
        "); background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
    stylesheet = stylesheet+QType + \
        ":checked:pressed { image: url(icons/"+iconname+"_off_hovered."+filetype + \
        ");  background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
    stylesheet = stylesheet+QType + \
        ":unchecked:pressed { image: url(icons/"+iconname+"_on_hovered."+filetype + \
        ");  background-color: #3daee9;padding-top: -15px;padding-bottom: -17px;}"
    QElement.setStyleSheet(stylesheet)
...
class SoftInputWidget(QtWidgets.QDialog):
    ...
    def buttonClicked(self, char_ord):
        ...
        if char_ord in self.NO_ORD_KEY_LIST:
            keyPress = QtWidgets.QKeyEvent(
                QtCore.QEvent.KeyPress, char_ord, QtCore.Qt.NoModifier, '')
        else:
            keyPress = QtWidgets.QKeyEvent(
                QtCore.QEvent.KeyPress, char_ord, QtCore.Qt.NoModifier, chr(char_ord))
        ...
        if char_ord not in self.NO_ORD_KEY_LIST:
            keyRelease = QtWidgets.QKeyEvent(
                QtCore.QEvent.KeyPress, char_ord, QtCore.Qt.NoModifier, '')
    ...
    def update_panel_position(self):
        ...
        widget_bottom = widget.mapToGlobal(QtCore.QPoint(
            widget.frameGeometry().x(), widget.frameGeometry().y())).y()
...
class numericInputWidget(TouchInputWidget):
    def __init__(self):
        TouchInputWidget.__init__(self)

        self.numberStr = QtWidgets.QLineEdit()
        # actiate touch input
        self.numberStr.keyboard_type = 'numeric'

        gl = QtWidgets.QVBoxLayout()
        gl.addWidget(self.numberStr)
        self.setLayout(gl)
